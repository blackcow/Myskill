...
source_url: "https://arxiv.org/abs/2602.07055"
...
captured_at: "2026-05-13T16:53:04+08:00"
parser: "docling"
page_count: 34
content_chars: 125215
asset_count: 33
picture_count: 31
markdown_image_refs: 33
image_scale: 4.0
image_effective_dpi: 288
figure_fallback_count: 13
canonical_source: "paper.md"
status: "raw"
---

![Image](assets/image_000000_026ecf6965d20beb982091f54694e2dff3bfad2324f38325baa0beb54c7260b7.png)

## THEORY OF SPACE: CAN FOUNDATION MODELS CONSTRUCT SPATIAL BELIEFS THROUGH ACTIVE EXPLORATION?

Pingyue Zhang 1 , ∗ , † , Zihan Huang ∗ , Yue Wang 4 , ∗ , Jieyu Zhang 3 , ∗ , Letian Xue 1 , Zihan Wang 1 , Qineng Wang 1 , Keshigeyan Chandrasegaran 2 , Ruohan Zhang 2 , Yejin Choi 2 , Ranjay Krishna 3 , Jiajun Wu 2 , Li Fei-Fei 2 , Manling Li 1 , †

1 Northwestern University 2 Stanford University 3 University of Washington 4 Cornell University pingyuezhang@u.northwestern.edu, manling.li@northwestern.edu
...
![Image](assets/image_000001_70fd7d88198670e4a3e3d6b7a3be9b9086e306a41bf167b4ead76609b5a32402.png)

![Image](assets/image_000002_7e7911aef59d9cdd4b5add18aa03bbddc632f67aeb077007878ae6942fcefc72.png)

![Image](assets/image_000003_f32c8f5a416e8e7e694be4c5d33986ace3678927e7e39a966b09166a7a609676.png)
...
https://huggingface.co/datasets/MLL-Lab/tos-data
...
![Figure fallback page 2](assets/figure_fallback_page_0002_1_scale_4.png)
...
We apply THEORY OF SPACE to evaluate multimodal language models, which are promising candidates for embodied agents. By integrating vision and language, they support unified perception, reasoning, and action over time, yet existing foundation-model benchmarks offer little insight into these capabilities. Most current benchmarks fall into two categories: passive (Weston et al., 2015; Shi et al., 2022; Yang et al., 2025c; Gholami et al., 2025; Yang et al., 2025a), where the agent is only asked to reason over given observations, and task-driven (Gordon et al., 2018; Shridhar et al., 2020b; Li et al., 2025; Yang et al., 2025b), where the agent must achieve a specific goal (e.g., 'fi nd the red chair ').
...
sentational breakdowns, and evaluate whether an agent truly 'knows what is uncertain' rather than merely producing plausible outputs.
...
/book Definition: THEORY OF SPACE

Ability to construct , revise , and exploit an internal spatial belief.

Here, an internal spatial belief is a mental model (Taylor &amp; Tversky, 1992) of spatial layout and relations maintained in working memory and updated from partial observations. We formalize THEORY OF SPACE within a partially observable framework over a spatial structure S ∈ S . The agent interacts with S to generate a history h t = ( o 0: t , a 0: t ) , where o and a denote observations and actions. We define THEORY OF SPACE as the capacity to manipulate a probabilistic belief B t through three core operations:

1. Construct: To form a globally consistent internal spatial belief by actively seeking out and integrating partial observations. Formally, the agent integrates h t to approximate the true posterior, denoted as B t ( S ) ≈ P ( S | h t ) .
2. Revise: To dynamically update the internal belief by using new information acquired through further exploration to resolve conflicts with prior beliefs. Upon an environmental shift S → S ′ , the agent utilizes exploratory actions ∆ h to minimize the divergence from the new ground truth, i.e., B t +∆ t → P ( S ′ | h t +∆ t ) .
3. Exploit: To utilize the current belief to support spatial tasks. The agent utilizes a policy π conditioned on the belief, π ( a t | B t ) , to perform a downstream task T . In a benchmark context, we measure the value of belief by the performance metric J achieved by this policy: J ( π ( ·| B t ) , T ) .
...
![Figure fallback page 6](assets/figure_fallback_page_0006_3_scale_4.png)
...
|---------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Static              | Pairwise Relation ( direction ) report allocentric direction and distance from A to B .                                                                                                                                                               | Allocentric Mapping ( alloc.map ) predict global coordinates (and headings) for all objects.                                                                                                                                       |
| Forward Dynam- ics  | Perspective Taking ( persp.take ) output the observation from a specified object's perspective. Action-to-View ( act2view ) given a sequence of Goto / Rotate , pre- dict the final observation (one object in FOV with ego direction/distance bins). | Mental Rotation ( ment.rot ) predict the sequence of front-facing ob- jects during a 360 ◦ self-rotation. Location2View ( loc2view ) given a global pose, predict the observa- tion (one object in FOV with ego bins/dis- tances). |
| Backward Dynam- ics | Perspective Decision ( perc.dec ) infer which object's perspective the agent is currently adopting. View-to-Action ( view2act ) recover an action sequence that produces a target observation.                                                        | View2Location ( view2loc ) localize the agent (and optionally orienta- tion) from a target observation under the map.                                                                                                              |
...
(calculated by AC-3 algorithm). The efficiency is calculated as E = 1 -∑ N i =1 log 2 max(1 ,C i ) N log 2 M . This score ranges from 0 (no information gained, C i = M ) to 1 (all objects perfectly localized, C i = 1 ). Note that it can also be used to calculate the accumulated information gain at each step. Information gain is mainly used in text-based environments, since vision-based environments have direct access to scenes without such ambiguity. Therefore, for vision-based environments, we directly use node coverage to measure exploration efficiency.
...
|                    |                  | direction Static (S)   | persp.take perc.dec. act2view view2act   | persp.take perc.dec. act2view view2act   | persp.take perc.dec. act2view view2act   | persp.take perc.dec. act2view view2act   | alloc.map ment.rot loc2view   | alloc.map ment.rot loc2view   | alloc.map ment.rot loc2view   | alloc.map ment.rot loc2view   |                  |
|--------------------|------------------|------------------------|------------------------------------------|------------------------------------------|------------------------------------------|------------------------------------------|-------------------------------|-------------------------------|-------------------------------|-------------------------------|------------------|
| Methods            | Avg.step         |                        | Route                                    | Route                                    | Route                                    | Route                                    |                               | Dynamic (D) Survey            | Dynamic (D) Survey            | Dynamic (D) Survey            | Avg.             |
|                    |                  |                        | Vision-based World                       | Vision-based World                       | Vision-based World                       | Vision-based World                       |                               |                               |                               |                               |                  |
| Proprietary Models |                  |                        |                                          |                                          |                                          |                                          |                               |                               |                               |                               |                  |
...
| Text-based World   | Text-based World | Text-based World       | Text-based World                         | Text-based World                         | Text-based World                         | Text-based World                         | Text-based World              | Text-based World              | Text-based World              | Text-based World              | Text-based World |
| Proprietary Models |                  |                        |                                          |                                          |                                          |                                          |                               |                               |                               |                               |                  |
...
| Methods            | direction Static (S)   | persp.take         | perc.dec Dynamic   | act2view (D)       | view2act           | alloc.map Static (S)   | ment.rot           | loc2view Dynamic   | view2loc (D)       |                    |
|--------------------|------------------------|--------------------|--------------------|--------------------|--------------------|------------------------|--------------------|--------------------|--------------------|--------------------|
|                    |                        | Route              | Route              | Route              | Route              |                        | Survey             | Survey             | Survey             | Avg.               |
|                    |                        |                    | Vision-based       |                    | World              |                        |                    |                    |                    |                    |
...
## /lightbulb Key Findings: Active Exploration as the Bottleneck
...
| Methods            | ori. pos. overall   | Correctness        | ( % )              | ori. pos. Perception   | ( % )              | ori. pos. Local ↔ Global   | ( % )              | ori. pos. Stability ( % )   | ori. pos. Stability ( % )   | ori. pos. Self- tracking   | ( % )              | Uncertainty ( % )   |
|--------------------|---------------------|--------------------|--------------------|------------------------|--------------------|----------------------------|--------------------|-----------------------------|-----------------------------|----------------------------|--------------------|---------------------|
| Vision-based World | Vision-based World  | Vision-based World | Vision-based World | Vision-based World     | Vision-based World | Vision-based World         | Vision-based World | Vision-based World          | Vision-based World          | Vision-based World         | Vision-based World | Vision-based World  |
...
| Text-based World   | Text-based World    | Text-based World   | Text-based World   | Text-based World       | Text-based World   | Text-based World           | Text-based World   | Text-based World            | Text-based World            | Text-based World           | Text-based World   | Text-based World    |
...
## /lightbulb Key Findings: Cognitive Map Failures (Orientation, Stability, and Belief Drift)
...
## /lightbulb Key Findings: Maps as a Diagnostic Proxy
...
- Identification Accuracy ( F 1 ): How precisely the agent pinpoints which objects changed. We compute the F 1 score for detecting the subset of objects whose position or orientation shifted.
...
- Belief Inertia: Whether updating remains systematically biased toward obsolete priors. To quantify attraction back to pre-shift beliefs, we test whether the residual error of the updated belief aligns with the direction of the old belief. For each shifted object i , let b old i denote the pre-shift belief, b new i the post-revision belief, and g new i the post-shift ground truth. Define the prior-offset and post-revision error vectors: v i = b old i -g new i , e i = b new i -g new i . We define positional inertia as
...
## /lightbulb Key Findings: Vision Deficiencies &amp; Belief Inertia
...
| Methods            | all Avg.           | red. Steps ↓       | ori. Identification   | pos. ( % )         | ori. Belief Correctness   | pos. ( % ) ↑       | ori. Belief Inertia   | pos. % ) ↓         |
|--------------------|--------------------|--------------------|-----------------------|--------------------|---------------------------|--------------------|-----------------------|--------------------|
| Text-based World   | Text-based World   | Text-based World   | Text-based World      | Text-based World   | Text-based World          | Text-based World   | Text-based World      | Text-based World   |
| GPT-5.2            | 6.92               | 0.55               | 97.9                  | 98.4               | 89.5                      | 69.7               | 5.5                   | 12.5               |
| GEMINI-3 PRO       | 7.79               | 0.18               | 98.7                  | 98.8               | 91.8                      | 72.9               | 7.9                   | 5.7                |
| Vision-based World | Vision-based World | Vision-based World | Vision-based World    | Vision-based World | Vision-based World        | Vision-based World | Vision-based World    | Vision-based World |
...
Passive Spatial Reasoning. Early paradigms treat spatial reasoning as static inference: given a textual description, agents answer relational queries (Weston et al., 2015; Shi et al., 2022; Mirzaee et al., 2021; Li et al., 2024). Other benchmarks probe understanding from a single image, asking for relative directions, topological relations, or metric attributes (Ma et al., 2024; Deng et al., 2025; Cheng et al., 2024; Chen et al., 2024; Liao et al., 2024; Kamath et al., 2023). Multi-view and video benchmarks raise difficulty by requiring cross-view integration, egocentric-allocentric conversion, and temporal consistency (Yang et al., 2025c; Xu et al., 2025; Wu et al., 2025; Yeh et al., 2025; Gholami et al., 2025; Zhou et al., 2025b). Recent works explicitly adopt cognitive maps: VSI-Bench (Yang et al., 2025a) shows map formation improves video QA, and MindCube (Yin et al., 2025) demonstrates that predicting layouts boosts multi-view reasoning. While informative, these benchmarks remain disembodied, as agents reason only over pre-collected trajectories.
...
- OpenAI. Gpt-5.2 system card. https://cdn.openai.com/pdf/ 3a4153c8-c748-4b71-8e31-aecbde944f8d/oai\_5\_2\_system-card.pdf , August 2025. System card.
...
- Zhipu AI Team. Glm-4.6v: Native multimodal foundation model. Technical report / model release, 2025. URL https://z.ai/blog/glm-4.6v . Available online; native multimodal vision + reasoning model (128k context) among open-source LLMs.
...
![Image](assets/image_000011_ece90941ccf0b68c4ed0eacf8bb6cd825326f3b7e33e4215a02fb43d4871fde4.png)
...
![Image](assets/image_000012_cf401f56f94a9e2dd7b743cd4254ed38476edf26bbe0b02a1ebdd6b0ce91035f.png)

- Created with Try for free at moqups.com (b) Distribution of all 3D models used in our vision tasks.
...
![Image](assets/image_000013_d6ac2e212a956844a18f144918bc7127bfb39ca5a43c3eda280588f07e0d62df.png)
...
![Image](assets/image_000014_80de173a6c609d58730fdf38e9c7003e7e355640480e31fb520822b8a0c9bc8f.png)
...
![Figure fallback page 23](assets/figure_fallback_page_0023_10_scale_4.png)
...
You return to your starting position and face north. From a Top-Down map, describe where {obj\_name} is relative to {anchor\_name}.
...
Now you jump to {anchor\_name}'s direction, facing its direction. Describe where {obj\_name} is relative to you.
...
You return to your starting position and face north. Then you have executed an action sequence and changed to a new location and facing direction. You observe the following: {final\_obs}
...
Treat your starting position as the origin (0, 0) while facing north. Report allocentric coordinates using (x right/east, y up/north). Objects: {object\_list}.
...
You return to your starting position and face north. You will perform a full 360-degree rotation by continuously turning {turn\_direction} in place. Assume all walls are removed (you can see through walls), so every object is visible.

Focus on this set of objects: {object\_pool}. List them in the exact order they appear directly ahead while you rotate. If two objects share a bearing, place the nearer one first.

Answer format: &lt;object\_name1&gt;, &lt;object\_name2&gt;, ...

Example: mug, sofa, plant

## Location2View

{origin\_instruction}You move to {loc} and face {direction}.
...
You move to a new location and face {orientation}. {observations}

{origin\_instruction}What is your new 2D coordinate (x, y)?


![Figure fallback page 24](assets/figure_fallback_page_0024_11_scale_4.png)

Figure 11: Evaluation prompt design. We show the prompt for each evaluation task.


![Figure fallback page 25](assets/figure_fallback_page_0025_12_scale_4.png)
...
![Figure fallback page 25](assets/figure_fallback_page_0025_13_scale_4.png)
...
| Methods                    | direction Static (S)   | persp.take       | perc.dec Dynamic Route   | act2view (D)     | view2act         | alloc.map Static (S)   | ment.rot Survey   | loc2view Dynamic   | view2loc (D)     | Avg.             |
|----------------------------|------------------------|------------------|--------------------------|------------------|------------------|------------------------|-------------------|--------------------|------------------|------------------|
|                            |                        |                  |                          | Vision-based     | World            |                        |                   |                    |                  |                  |
| Proprietary Models GPT-5.2 | 39.2                   | 37.3             | 63.3                     | 53.8             | 58.3             | 68.2                   | 92.7              | 52.3               | 68.6             | 59.3             |
...
| Methods            | Avg.cost           | direction persp.take perc.dec. Static (S)   | direction persp.take perc.dec. Static (S)   | act2view view2act Dynamic Route   | (D)                | act2view view2act Dynamic Route   | alloc.map Static (S)   | ment.rot loc2view view2loc Dynamic (D) Survey   | ment.rot loc2view view2loc Dynamic (D) Survey   | ment.rot loc2view view2loc Dynamic (D) Survey   | Avg.               |
|--------------------|--------------------|---------------------------------------------|---------------------------------------------|-----------------------------------|--------------------|-----------------------------------|------------------------|-------------------------------------------------|-------------------------------------------------|-------------------------------------------------|--------------------|
|                    |                    |                                             |                                             | Vision-based                      |                    |                                   |                        |                                                 |                                                 |                                                 |                    |
...
| Methods                    | direction persp.take Static (S)   | direction persp.take Static (S)   | perc.dec act2view view2act Dynamic (D)   | perc.dec act2view view2act Dynamic (D)   | perc.dec act2view view2act Dynamic (D)   | alloc.map Static (S)   | ment.rot loc2view view2loc Dynamic (D) Survey   | ment.rot loc2view view2loc Dynamic (D) Survey   | ment.rot loc2view view2loc Dynamic (D) Survey   | Avg.             |
|----------------------------|-----------------------------------|-----------------------------------|------------------------------------------|------------------------------------------|------------------------------------------|------------------------|-------------------------------------------------|-------------------------------------------------|-------------------------------------------------|------------------|
|                            |                                   |                                   |                                          | Vision-based                             | World                                    |                        |                                                 |                                                 |                                                 |                  |
| Proprietary Models GPT-5.2 | 47.0                              | 37.7                              | 59.7                                     | 38.3                                     | 40.3                                     | 60.1                   | 73.7                                            | 50.5                                            | 65.9                                            | 52.6             |
| GEMINI-3 PRO               | 63.5                              | 35.5                              | 58.7                                     | 42.8                                     | 43.0                                     | 64.4                   | 81.7                                            | 48.8                                            | 67.4                                            | 56.2             |
| Text-based World           | Text-based World                  | Text-based World                  | Text-based World                         | Text-based World                         | Text-based World                         | Text-based World       | Text-based World                                | Text-based World                                | Text-based World                                | Text-based World |
| Proprietary Models GPT-5.2 | 83.8                              | 88.2                              | 94.3                                     | 86.8                                     | 62.7                                     | 94.8                   | 93.7                                            | 82.0                                            | 92.5                                            | 86.5             |
...
| Methods            | Avg.cost           | direction persp.take Static (S)   | direction persp.take Static (S)   | perc.dec. act2view view2act Dynamic Route   | (D)                | perc.dec. act2view view2act Dynamic Route   | alloc.map Static (S)   | ment.rot loc2view view2loc Dynamic (D) Survey   | ment.rot loc2view view2loc Dynamic (D) Survey   | ment.rot loc2view view2loc Dynamic (D) Survey   | Avg.               |
|--------------------|--------------------|-----------------------------------|-----------------------------------|---------------------------------------------|--------------------|---------------------------------------------|------------------------|-------------------------------------------------|-------------------------------------------------|-------------------------------------------------|--------------------|
|                    |                    |                                   |                                   | Vision-based                                |                    |                                             |                        |                                                 |                                                 |                                                 |                    |
| Proprietary Models | Proprietary Models | Proprietary Models                | Proprietary Models                | Proprietary Models                          | Proprietary Models | Proprietary Models                          | Proprietary Models     | Proprietary Models                              | Proprietary Models                              | Proprietary Models                              | Proprietary Models |
...
![Image](assets/image_000018_14229830f58190ae9bc6c4eac60f60af0b3e5c071324f9d680fa3cfe3f7a4e90.png)

## Pairwise Direction

Q: You return to your starting position and face north.

From a Top-Down map, describe where shelf is relative to truck.

- A: south east, mid distance

## Perspective Taking

- Q: Now you jump to backpack's direction, facing its direction.
...
2. Rotate(-90)
3. Jump to the object at front-right, mid distance. 4. Rotate(-180)
...
Q: You return to your starting position and face north. Then you have executed an action sequence and changed to a new location and facing direction. You observe the following: pan is at front-right, slightly far, facing backward; truck is at front-right, mid distance, facing forward; laptop is at front, mid distance, facing backward What action sequence led to this final view?

- A: [['rotate', 90], ['jumpto', 'green door'], ['jumpto', 'shelf'], ['rotate', 180]]


![Figure fallback page 28](assets/figure_fallback_page_0028_14_scale_4.png)
...
Q: You move to a new location and face north. You observe: pan is at front, mid distance, facing right; truck is at front-right, mid distance, facing left; green door is at front-slight-left, slightly far, on left wall Treat the green door as the new 'origin' (0, 0).


![Figure fallback page 29](assets/figure_fallback_page_0029_15_scale_4.png)

Figure 15: GPT-5.2's turn-by-turn cognitive map in text world during exploration.


![Figure fallback page 29](assets/figure_fallback_page_0029_16_scale_4.png)

Figure 16: GPT-5.2's turn-by-turn cognitive map in vision world during exploration.


![Image](assets/image_000021_6a50a7aa43c66f574abd937ead8a29bdd3282788ef5e3fe2ff6e56479fea9960.png)
...
![Image](assets/image_000022_c67b71ae86f04adeb6e1e8abfd218a29761b17f986ad78c03c201831a3953048.png)
...
![Image](assets/image_000023_6bae814fd5a6f5aabfc2f2daf0a24b2d70fe8f9660c095b848b247110ab061bc.png)

Figure 19: Example trajectory illustrating GEMINI-3 PRO's door-finding strategy and systematic sweeping pattern in vision world: Upon detecting a door, the agent navigates toward it and executes a strategic rotation to maximize environmental coverage. The process terminates once all target objects have been successfully identified.

![Image](assets/image_000024_1a1a714e840a4a3caae277bad11cadedb316cd842db53bb75d7b4d8ca5930e03.png)
...
![Image](assets/image_000025_40ccb0b7e6dec9d9153e946e6676e7b63db4c0d251d286dd4de2fc9d0390486c.png)

Figure 21: Example trajectory illustrating CLAUDE-4.5 SONNET's exploration pattern: There is no clear exploration pattern.

![Image](assets/image_000026_9db9c903407191ba56a1f34855902c493f7eb405ba2a9808f65af95fbf3e1ab9.png)

Figure 22: Platform designed by us for analysis (chart)

![Image](assets/image_000027_a49e29c0333e79a712fe2b00a1b32743d41d85d65430bb7119efdc02c2797b98.png)

Figure 23: Visualization Platform for analysis: Metrics for active exploration in text world

![Image](assets/image_000028_35ce59723e16361cdf20f8cea37712381d86f5cd235742a6be8664b62c18971f.png)

Figure 24: Visualization Platform for analysis: Metrics for active exploration in vision world

![Image](assets/image_000029_fc41bdab4275178c3263b53877a13d59487e14aae57d28d5d99f88230d617b3a.png)
...
![Image](assets/image_000030_708c175321a0254bed40160341d52bc42b87c76a9c56748231651af119758375.png)
